...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      controller.loadAwards(i);
      controller.loadPastPerform(i);
    }
    ...
    return SingleChildScrollView(
      child: Column(children: [
        ...
        Container(
          ...
          child: ListView.builder(
              scrollDirection: Axis.horizontal,
              itemCount: controller.awards[ct].length,
              itemBuilder: (context, index) {
                return controller.awards[ct].isNotEmpty
                    ? GestureDetector(
                        onTap: () {
                          Get.to(()=> DetailPosterPage(id:controller.awards[ct][index].id));
                        },
                        child: Container(
                            clipBehavior: Clip.hardEdge,
                            margin: EdgeInsets.only(right: 20.w),
                            ...
                            child: Get.find<Controller>()
                                    ...
                                    .contains('http')
                                ? Image.network(
                                    controller.awards[ct][index].image,
                                    fit: BoxFit.cover)
                                : Image.network(
                                    "http://www.kopis.or.kr" +
                                        controller.awards[ct][index].image,
                ...
                return Get.find<Controller>().poster[ct].isNotEmpty
                    ? GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: controller.poster[ct][index].image.contains("http")?
                          Image.network(
        ...
        Align(
            alignment: Alignment.topLeft,
            child: Text("past".tr.toUpperCase()+"perform".tr.toUpperCase(), style: TextStyle(fontWeight: FontWeight.w900,fontSize: 20.w))),
        ...
        Container(
          ...
          child: ListView.builder(
              scrollDirection: Axis.horizontal,
              itemCount: Get.find<Controller>().completed[ct].length,
              itemBuilder: (context, index) {
                return Get.find<Controller>().completed[ct].isNotEmpty
                    ? GestureDetector(
                  onTap: () {
                    Get.to(()=> DetailPosterPage(id:Get.find<Controller>().completed[ct][index].id));
                  },
                  child: Container(
                    clipBehavior: Clip.hardEdge,
                    margin: EdgeInsets.only(right: 20.w),
                    ...
                    child: controller.completed[ct][index].image.contains('http')?
                    Image.network(
                      Get.find<Controller>().completed[ct][index].image,
                      fit: BoxFit.cover,
                    ):Image.network(
                      "http://www.kopis.or.kr" +
                      Get.find<Controller>().completed[ct][index].image,
